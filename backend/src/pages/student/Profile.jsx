...
useProfileSocket(profile.id, (message) => {
  if (!message || message.type !== "PROFILE_UPDATED") return;

  const updatedProfile = message.data;

  if (!updatedProfile || !updatedProfile.id) return;

  if (isUpdating) return;

  // ✅ PREVENT OLD DATA OVERWRITE
  if (updatedProfile.updated_at && profile.updated_at) {
    if (new Date(updatedProfile.updated_at) < new Date(profile.updated_at)) {
      return;
    }
  }
  ...
  setOtpState(prev => ({
    ...prev,
    emailVerified: updatedProfile.email_verified ?? prev.emailVerified,
    mobileVerified: updatedProfile.mobile_verified ?? prev.mobileVerified
  }));
});
...
const handleResumeChange = async (e) => {
  ...
  // 🚨 FORCE SAVE BEFORE UPLOAD
  if (isProfileChanged) {
    const confirmSave = window.confirm(
      "You have unsaved changes. Save before uploading resume?"
    );

    if (confirmSave) {
      await handleUpdate(); // ✅ SAVE PROFILE FIRST
    } else {
      return;
    }
  }
